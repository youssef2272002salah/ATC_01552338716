...
class MailService {
    ...
    async sendEmail({ toEmail, subject, htmlContent }) {
        ...
        try {
            ...
            logging_1.logger.info(`✅ Email sent to: ${toEmail}`);
        }
        catch (error) {
            logging_1.logger.error(`❌ Error sending email to ${toEmail}:`, error);
        }
    }
    generateVerificationEmail(toEmail, verificationLink) {
        return {
            toEmail,
    ...
    generateResetPasswordEmail(toEmail, resetToken) {
        return {
            toEmail,
